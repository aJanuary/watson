...
public class MembershipModule implements EventListener {
                          ...
                          .filter(
                              m ->
                                  m.getRoles().stream()
                                      .noneMatch(
                                          r ->
                                              r.getId().equals(unverifiedRole.getId())
                                                  || r.getId().equals(membersRole.getId())))
                          .map(Member::getId)
                          .toList()));
                  ...
                  try {
                    ...
                    if (userDetails.isNull()) {
                      ...
                      for (var roleName : roles) {
                        ...
                        guild
                            .addRoleToMember(
                                UserSnowflake.fromId(userId), jdaUtils.getRole(roleName))
                            .queue(
                                (v) -> {},
                                e ->
                                    logger.error(
                                        "Error adding role {} to user {}", roleName, userId, e));
                      }
                    }
                  } catch (Exception e) {
  ...
  private String calculateSignature(String dataToSign) {
    try {
      ...
      var secretKey =
          new SecretKeySpec(
              membershipConfig.membersApiKey().getBytes(StandardCharsets.UTF_8), "HmacSHA256");
      mac.init(secretKey);
